...
export function openCountrySquad(code, origin = 'squads') {
  ...
  if (!t.squad || t.squad.length === 0) {
    ...
    t.squad.forEach(p => {
      ...
      const photoUrl = getPlayerPhotoUrl(p.name);
      const photoEl = photoUrl ? `<img src="${photoUrl}" referrerpolicy="no-referrer" style="width: 60px; height: 60px; border-radius: 50%; object-fit: cover; border: 1px solid var(--border-glass);" onerror="this.style.display='none'">` : '';

      row.innerHTML = `
        <td>
          <div class="player-name-cell" style="display: flex; align-items: center; gap: 0.5rem;">
            ${photoEl}
...
function renderLineup(team) {
  ...
  const createPlayerHTML = (p, num) => {
    ...
    const photoHtml = photoUrl
      ? `<img class="sofa-photo-circle" src="${photoUrl}" referrerpolicy="no-referrer" style="object-fit: cover; object-position: top center; border: 1.5px solid rgba(255,255,255,0.4);" onerror="this.style.display='none'"/>`
      : `<div class="sofa-photo-circle"><span>${initials.toUpperCase()}</span></div>`;
